...
이미지는 위젯을 처음 표시해야 할 때 지연 로딩됩니다. 기본 설정에서는 유효한 Advisor 메시지가 도착할 때, `alwaysVisible`을 켠 경우에는 세션 시작 시 불러옵니다. 확정된 각 로컬 이미지 경로에는 재시도 가능한 Promise 캐시를 따로 사용합니다. 성공한 로드는 재사용하고 실패한 로드는 캐시에서 제거하므로, 파일을 수정한 뒤 다시 불러올 수 있습니다. 잘못된 이미지 재정의 경로를 설정하면 활성화할 때마다 경고를 한 번 출력하고 번들 에셋으로 대체합니다. 번들 에셋도 불러오지 못하면 위젯을 표시하지 않습니다.
